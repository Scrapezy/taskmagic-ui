...
export interface FileWatcherEvents {
    ...
    'task-changed': (task: Task, action: 'added' | 'modified' | 'deleted') => void;
    'plan-changed': (plan: Plan, action: 'added' | 'modified' | 'deleted') => void;
...
export class FileWatcher extends EventEmitter {
     ...
     */
    private handleFileChange(filePath: string, action: 'added' | 'modified' | 'deleted'): void {
     ...
     */
    private processFileChange(filePath: string, action: 'added' | 'modified' | 'deleted'): void {
        try {
            ...
            if (relativePath === '.ai/TASKS.md') {
                this.handleMasterTaskFileChange(action);
                return;
            }
            ...
            this.emit('error', new Error(`Failed to process file change: ${error}`));
        }
    }
     ...
     */
    private handleTaskFileChange(filePath: string, action: 'added' | 'modified' | 'deleted'): void {
        try {
            if (action === 'deleted') {
     ...
     */
    private handlePlanFileChange(_filePath: string, action: 'added' | 'modified' | 'deleted'): void {
        try {
            if (action === 'deleted') {
                this.reloadProject();
                return;
     ...
     */
    private handleMasterTaskFileChange(_action: 'added' | 'modified' | 'deleted'): void {
    ...
    private reloadProject(): void {
        try {
            ...
            this.emit('error', new Error(`Failed to reload project: ${error}`));
        }
    }
