...
export class TaskMagicParser {
   ...
   */
  static parseYamlFrontmatter(content: string): {
   ...
   */
  static validateTaskFrontmatter(
    frontmatter: Record<string, unknown>
  ): TaskStatus {
    const required = [
      'id',
    ...
    if (!validPriorities.includes(frontmatter['priority'] as string)) {
      throw new Error(
        `Invalid priority: ${frontmatter['priority']}. Must be one of: ${validPriorities.join(', ')}`
      );
    }
    ...
    return frontmatter as unknown as TaskStatus;
  }

  /**
   * Parse markdown content to extract description, details, and test strategy
   */
  static parseTaskBody(body: string): {
    description: string;
    details: string[];
    testStrategy: string;
    agentNotes?: string;
    ...
    function visit(taskId: string | number): boolean {
      ...
      if (task) {
        for (const depId of task.dependencies) {
          if (visit(depId)) {
            errors.push(
              `Circular dependency detected: Task ${taskId} -> Task ${depId}`
            );
          }
        }
      }
