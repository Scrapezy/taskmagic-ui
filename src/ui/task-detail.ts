...
export class TaskDetailView {
    ...
    private sections: (blessed.Widgets.BoxElement | blessed.Widgets.ListElement)[] = [];
    private breadcrumbs: Task[] = [];
    private onExit: () => void;

    constructor(project: TaskMagicProject, task: Task, breadcrumbs: Task[] = [], onExit?: () => void) {
    ...
    private createLayout(): void {
        ...
        // Dependency list for navigation
        ...
        // Store sections for tab navigation
        this.sections = [
            this.taskInfoBox,
            this.dependencyList,
            this.descriptionBox,
    ...
    private updateBreadcrumb(): void {
        const breadcrumbText = this.breadcrumbs
            .map((task, index) => {
                ...
                return isLast ? `${prefix}{bold}${taskText}{/bold}` : `${prefix}${taskText}`;
            })
            .join('');
...
Created: ${this.formatDate(this.task.created_at)}  Started: ${this.formatDate(this.task.started_at) || '{gray-fg}Not started{/gray-fg}'}  Completed: ${this.formatDate(this.task.completed_at) || '{gray-fg}Not completed{/gray-fg}'}`;
    ...
    private updateTaskInfo(): void {
        ...
        if ((this.task as any).agent_notes) {
            content += `\n{bold}Agent Notes:{/bold}\n`;
            content += `──────────────\n`;
            content += `${(this.task as any).agent_notes}\n`;
        }
    ...
    private updateDescription(): void {
        ...
        content += this.task.description || '{gray-fg}No description available{/gray-fg}';
    ...
    private updateTestStrategy(): void {
        ...
        content += this.task.testStrategy || '{gray-fg}No test strategy defined{/gray-fg}';
    ...
    private navigateToDependency(): void {
        const selectedIndex = (this.dependencyList as any).selected;
        const dependencies = this.getDependencyTasks();
        const dependents = this.getDependentTasks();
        const allNavigableTasks = [...dependencies, ...dependents];
    ...
    private navigateToTask(task: Task): void {
        ...
        setTimeout(() => {
            const detailView = new TaskDetailView(this.project, task, this.breadcrumbs, this.onExit);
            detailView.run();
        }, 10);
    }
    ...
    private showBreadcrumbMenu(): void {
        ...
        if (this.breadcrumbs.length > 1) {
            ...
            const items = this.breadcrumbs.map(task => `[${task.id}] ${task.title}`);
            menu.setItems(items);

            menu.key(['enter'], () => {
                const selectedIndex = (menu as any).selected;
    ...
    private navigateToTaskInBreadcrumb(task: Task, breadcrumbIndex: number): void {
        // Clean up current screen
        this.screen.destroy();
        ...
        setTimeout(() => {
            ...
            const detailView = new TaskDetailView(this.project, task, newBreadcrumbs.slice(0, -1), this.onExit);
            detailView.run();
        }, 10);
    }
    ...
    private navigateBack(): void {
        if (this.breadcrumbs.length > 1) {
            ...
            if (parentTask) {
                this.navigateToTaskInBreadcrumb(parentTask, this.breadcrumbs.length - 2);
            } else {
                this.exitToParent();
            }
        } else {
