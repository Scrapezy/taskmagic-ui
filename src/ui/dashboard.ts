...
{cyan-fg}Version: 0.1.0{/cyan-fg}    {cyan-fg}Project: ${this.getProjectName()}{/cyan-fg}    {gray-fg}Press ? for help{/gray-fg}`;
    ...
    private updateDashboard(): void {
        ...
        // Health indicator
        const healthColor = this.getHealthColor(progressPercent, blockedTasks.length);
    ...
    private updateFilteredTasks(): void {
        ...
        filtered.sort((a, b) => {
            switch (this.currentSort) {
                ...
                case 'id':
                    const aId = typeof a.id === 'number' ? a.id : parseInt(a.id.toString(), 10) || 0;
                    const bId = typeof b.id === 'number' ? b.id : parseInt(b.id.toString(), 10) || 0;
                    return aId - bId;
                case 'status':
...
{center}Press ? again to close this help{/center}`;
    ...
    private showTaskDetail(): void {
        const selectedIndex = (this.taskTable as any).selected;
...
${dependentTasks.length > 0 ? `┌─ {bold}Dependent Tasks{/bold} ────────────────────────────────────────────────────────┐
...
{center}{bold}Navigation:{/bold} ↑/↓ Scroll • Tab Jump to Dependency • Escape Close{/center}`;
        ...
        popup.key(['tab'], () => {
            if (task.dependencies.length > 0) {
                this.screen.remove(popup);
                const firstDep = this.project.structure.tasks.find((t: Task) => t.id === task.dependencies[0]);
                if (firstDep) {
        ...
        for (let i = 1; i <= Math.min(9, task.dependencies.length); i++) {
            popup.key([i.toString()], () => {
                const depTask = this.project.structure.tasks.find((t: Task) => t.id === task.dependencies[i - 1]);
                if (depTask) {
                    this.screen.remove(popup);
    ...
    private buildDependencyInfo(task: Task): Array<{ task: Task, level: number }> {
        ...
        const traverse = (taskId: string | number, level: number) => {
            ...
            const depTask = this.project.structure.tasks.find((t: Task) => t.id === taskId);
            if (depTask) {
                result.push({ task: depTask, level });
                depTask.dependencies.forEach((depId: string | number) => traverse(depId, level + 1));
            }
        };

        task.dependencies.forEach((depId: string | number) => traverse(depId, 0));
        return result.sort((a, b) => a.level - b.level || Number(a.task.id) - Number(b.task.id));
    }
    ...
    private formatDependencyTree(deps: Array<{ task: Task, level: number }>): string {
        if (deps.length === 0) {
    ...
    private getHealthColor(progressPercent: number, blockedCount: number): string {
