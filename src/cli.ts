...
program
  ...
  .option(
    '-f, --filter <type>',
    'Filter tasks by status (pending, inprogress, completed, failed)'
  )
  .action(async (options) => {
    ...
    tasks.forEach((task: Task) => {
      ...
      const title =
        task.title.length > 44
          ...
          : task.title.padEnd(44);
      const deps =
        task.dependencies.length > 0 ? task.dependencies.join(', ') : 'None';
      const depsFormatted =
        deps.length > 12 ? deps.slice(0, 9) + '...' : deps.padEnd(12);

      console.log(
        `│ ${id} │ ${status} │ ${priority} │ ${title} │ ${depsFormatted} │`
      );
    });
...
program
  ...
  .action(() => {
    ...
    console.log(
      '  taskmagic-ui list -f pending           # List pending tasks only'
    );
    console.log(
      '  taskmagic-ui stats                     # Show project statistics'
    );
    console.log(
      '  taskmagic-ui -p /path/to/project       # Use specific project path'
    );
    console.log('');
    console.log(
      '📋 For more information visit: https://github.com/scrapezy/taskmagic-ui'
    );
  });
...
program
  ...
  .action(async (options) => {
    console.log(
      '⚠️  The "status" command is deprecated. Please use "stats" instead.'
    );
